...
Game.prototype = {
  init: function (currentLevel) {
    ...
    this.KILL_DIFF = 25;
    //Ofrece colisiones con cuerpos no cuadriláteros
  ...
  create: function () {
    ...
    this.floor = this.game.add.tileSprite(
    ...
    this.isPreparingShoot = false;
    this.game.input.onDown.add(this.prepareShoot, this);
    this.setupChicken();
  },
  prepareShoot: function () {
    if (this.isChickenReady) {
      this.isPreparingShoot = true;
      this.isChickenReady = false;
    }
  },
  loadLevel: function () {
  ...
  //Cuerpos, Forma de cuerpos, equation = Parametros(velocidad)
  hitEnemy: function (bodyA, bodyB, shapeA, shapeB, equation) {
    let velocityDiff = Phaser.Point.distance(
      new Phaser.Point(
        equation[0].bodyA.velocity[0],
        equation[0].bodyA.velocity[1]
      ),
      new Phaser.Point(
        equation[0].bodyB.velocity[0],
        equation[0].bodyB.velocity[1]
      )
    );
  ...
  throwChicken: function () {
    ...
    this.chicken.body.velocity.x =
      (Math.abs(diff.x * this.SHOOT_FACTOR) / (diff.x * this.SHOOT_FACTOR)) *
      Math.min(Math.abs(diff.x * this.SHOOT_FACTOR), this.MAX_SPEED_SHOOT);
    this.chicken.body.velocity.y =
      (Math.abs(diff.y * this.SHOOT_FACTOR) / (diff.y * this.SHOOT_FACTOR)) *
      Math.min(Math.abs(diff.y * this.SHOOT_FACTOR), this.MAX_SPEED_SHOOT);
