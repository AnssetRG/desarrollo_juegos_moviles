...
Enemy.prototype.damage = function (amount) {
  Phaser.Sprite.prototype.damage.call(this, ammount);
  this.play("getHit");
  if (this.health <= 0) {
    //Emisor de partículas (posx, posy, total de partículas)
    let emitter = this.game.add.emitter(this.x, this.y, 100);
    emitter.makeParticles("enemyParticle");
    emitter.minParticleSpeed.setTo(-200, -200);
    emitter.maxParticleSpeed.setTo(200, 200);
    emitter.gravity = 0;
    //si quieres que exploten, lifespan de las partículas, frecuencia (null en caso de que exploten), cuántas quieres por frecuencia
